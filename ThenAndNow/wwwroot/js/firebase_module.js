...
window.signInWithGoogle1 = async () => {
    const provider = new GoogleAuthProvider();
    try {
        const result = await signInWithPopup(auth, provider);
        return result;
    } catch (error) {
        console.error("window.signInWithGoogle1 error: ", error);
        return { error: error.message };
    }
};

export async function signInWithGoogle2() {
    const provider = new GoogleAuthProvider();
    try {
        const result = await signInWithPopup(auth, provider);
        return result;
    } catch (error) {
        console.error("signInWithGoogle2 error: ", error);
        return { error: error.message };
    }
}
